...
public class PlayerGameMasterTool : NetworkBehaviour
{
    ...
    // Event and GM Promotion //////////////////////////////////////////////////
    [Command]
    public void CmdStartEvent(string eventName)
    {
        // validate: only for GMs
        if (!player.isGameMaster) return;

        // TODO: Implement event starting logic here
    ...
    public void CmdPromoteToGM(string playerName)
    {
        ...
        if (Player.onlinePlayers.TryGetValue(playerName, out Player targetPlayer))
        {
            targetPlayer.isGameMaster = true;
            Debug.Log($"{playerName} has been promoted to GM.");
        }
    }

    [Command]
    public void CmdRevokeGM(string playerName)
    {
        // validate: only for GMs
        if (!player.isGameMaster) return;
        ...
        if (Player.onlinePlayers.TryGetValue(playerName, out Player targetPlayer))
        {
            ...
            Debug.Log($"{playerName}'s GM promotion has been revoked.");
        }
    }
